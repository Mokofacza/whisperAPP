...
class BatchWorker(QtCore.QObject):
    ...
    def run(self):
        ...
        for i, (audio_path, ref_path) in enumerate(self.items, 1):
            ...
            try:
                ...
                if ref_path and ref_path.is_file():
                    ...
                    if wer is not None:
                        sum_wer += wer; cnt_wer += 1; msg += f" | WER {wer:.1f}%"
                    else:
                        msg += " | WER —"
                    if cer is not None:
                        sum_cer += cer; cnt_cer += 1; msg += f" | CER {cer:.1f}%"
                    else:
                        msg += " | CER —"
                else:
        ...
        processed = max(1, i - 1 if self._stop else i)  # Poprawka na przerwane
        if processed == 0:
            self.line.emit("\nAnulowano przed przetworzeniem plików.")
            self.enable.emit(True);
            self.finished.emit();
            return
...
class FilesTab(QtWidgets.QWidget):
    def __init__(self):
        ...
        title = QtWidgets.QLabel("Transkrypcja plików");
        f = title.font();
        ...
        self.btnRun = QtWidgets.QPushButton("Transkrybuj");
        self.btnRun.setProperty("class", "Primary")
        self.btnRun.setIcon(style.standardIcon(QtWidgets.QStyle.SP_MediaPlay))

        self.btnStop = QtWidgets.QPushButton("STOP");
        self.btnStop.setProperty("class", "Danger");
        self.btnStop.setEnabled(False)
        self.btnStop.setIcon(style.standardIcon(QtWidgets.QStyle.SP_MediaStop))
        ...
        form.addWidget(self.lang, row, 3)

        row += 1
        form.addWidget(QtWidgets.QLabel("Wejście:"), row, 0)
        self.inputKind = QtWidgets.QComboBox();
        self.inputKind.addItems(["Folder", "Plik"]);
        form.addWidget(self.inputKind, row, 1)
        ...
        btnDir.setIcon(style.standardIcon(QtWidgets.QStyle.SP_DirIcon))
        w1 = QtWidgets.QWidget();
        h1 = QtWidgets.QHBoxLayout(w1);
        h1.setContentsMargins(0, 0, 0, 0);
        h1.addWidget(self.dir, 1);
        h1.addWidget(btnDir)
        ...
        btnFile.setIcon(style.standardIcon(QtWidgets.QStyle.SP_FileIcon))
        w2 = QtWidgets.QWidget();
        h2 = QtWidgets.QHBoxLayout(w2);
        h2.setContentsMargins(0, 0, 0, 0);
        h2.addWidget(self.file, 1);
        h2.addWidget(btnFile)
        ...
        self.exts = QtWidgets.QLineEdit("wav, mp3, m4a, flac, ogg");
        form.addWidget(self.exts, row, 1)
    ...
    def _set_enabled(self, en: bool):
        self.btnRun.setEnabled(en);
        self.btnStop.setEnabled(not en)
    ...
    def _build_items(self) -> Optional[List[Tuple[Path, Optional[Path]]]]:
        ...
        if not is_folder:
            ...
            if want_eval and ref and not ref.is_file():  # Sprawdzenie czy ref nie jest None
                QtWidgets.QMessageBox.warning(self, "Brak TXT", "Nie znaleziono pliku TXT obok audio.")
                ref = None  # Resetuj ref, jeśli nie znaleziono
            items.append((p, ref))
        else:
            ...
            if not exts: exts = AUDIO_SUFFIXES  # Użyj domyślnych
            ...
            for a in audios:
                ...
                if want_eval and ref and not ref.is_file():
                    ref = None  # Nie znaleziono TXT, traktuj jako brak
                items.append((a, ref))
    ...
    def _run(self):
        ...
        model_opts = get_model_args();
        model_opts["label"] = get_label()
        ...
        self.thread.finished.connect(self._on_finish)  # Upewnij się, że UI jest odblokowane
    ...
    def _stop(self):
        ...
        # self._set_enabled(True) # To zostanie wywołane przez 'enable' lub 'finished'
